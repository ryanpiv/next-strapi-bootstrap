...
module.exports = {
  ...
  /**
   * Creates an order and sets up the Stripe checkout session for the FE
   * @param {any} ctx
   */
  async create(ctx) {
    const { product } = ctx.request.body;

    if (!product) { return ctx.throw(400, 'Please specify a product')}

    const realProduct = await strapi.services.product.findOne({ id: product.id });

    if (!realProduct) { return ctx.throw(404, 'No product with such id' )};
    ...
    //Create the order
    const newOder = await strapi.services.order.create({
      user: user.id,
  ...
  async confirm(ctx) {
    ...
    if (session.payment_status === 'paid') {
      ...
      ctx.throw(400, "The Payement wasn't successful, please call support");
    }
  }
};
